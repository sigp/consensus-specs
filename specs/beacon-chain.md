...
* Verify that the given `justified_block_hash` is in the chain at `justified_slot`.
* Verify `justified_slot` is equal to or earlier than the `last_justified_slot` in the crystallized state.
